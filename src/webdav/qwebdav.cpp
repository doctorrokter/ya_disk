...
QString QWebdav::hostname() const { return m_baseUrl.host(); }
int QWebdav::port() const { return m_baseUrl.port(); }
QString QWebdav::rootPath() const { return m_rootPath; }
QWebdav::QWebdavConnectionType QWebdav::connectionType() const { return m_currentConnectionType; }
bool QWebdav::isSSL() const { return (m_currentConnectionType==QWebdav::HTTPS); }
...
QNetworkReply* QWebdav::put(const QString& path, QFile& file) {
    ...
    QString shaStr = sha1.result().toHex();

    QNetworkRequest req;
    req.setRawHeader("Accept", "*/*");
    req.setRawHeader(QString("Authorization").toLatin1(), QString("OAuth ").append(m_accessToken).toLatin1());
    req.setRawHeader("Etag", md5Str.toLatin1());
    req.setRawHeader("Sha256", shaStr.toLatin1());
    req.setRawHeader("Expect", "100-continue");
    req.setRawHeader("Content-Length", QString::number(bytes.size()).toLatin1());
    ...
    return QNetworkAccessManager::put(req, bytes);
}
